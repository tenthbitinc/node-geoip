...
var dataFiles = {
	...
	dma: path.join(geodatadir, 'adwords-dma-regions.json')
};
	...
	do {
		...
		if (floor <= ip && ceil >= ip) {
			...
			if (recordSize === RECORD_SIZE) {
				...
				geodata.dma = locBuffer.readInt16BE((locId * locRecordSize) + 12);
				geodata.city = locBuffer.toString('utf8', (locId * locRecordSize) + 14, (locId * locRecordSize) + locRecordSize).replace(/\u0000.*/, '');
				if (geodata.dma && Object.prototype.hasOwnProperty.call(cacheDMA, geodata.dma)) {
					geodata.dma_name = cacheDMA[geodata.dma];
				}
			}
...
function preloadDMA(callback) {
	// TODO: Not synchronous and not safe if file is missing
	jsonFile = fs.readFileSync(dataFiles.dma, {encoding: 'utf8'});
	cacheDMA = JSON.parse(jsonFile);
	console.log('dma', cacheDMA);
}
